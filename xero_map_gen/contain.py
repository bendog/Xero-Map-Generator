...
class XeroObjectGroup(object):
    @classmethod
    def dump_items_csv(
        cls, items, dump_path="items.csv", names=None, flatten_attr=None
    ):
        try:
            ...
            PKG_LOGGER.error(
                "file "
                + dump_path
                + " could not be opened. Try closing the file. "
                + repr(exc)
            )
            try:
...
class XeroContactGroup(XeroObjectGroup):
    ...
    def dump_contacts_sanitized_table(cls, contacts):
        return tabulate.tabulate(
            [contact.flatten_sanitized() for contact in contacts], headers="keys"
        )
...
class XeroContact(XeroObject):
    ...
    address_type_priority = ["STREET", "POBOX", "DELIVERY"]
    ...
    phone_type_priority = ["DEFAULT", "DDI", "MOBILE"]
    ...
    def main_phone(self):
        ...
        self._main_phone = self._primary_property(
            self.data.get("Phones", []),
            "PhoneType",
            self.phone_type_priority,
            phone_empty,
            default={},
        )
        return self._main_phone
    ...
    def phone(self):
        ...
        if not main_phone.get("PhoneAreaCode"):
            return response
        response = "%s %s" % (main_phone["PhoneAreaCode"], response)
        if not main_phone.get("PhoneCountryCode"):
            return response
        response = "(%s) %s" % (main_phone["PhoneCountryCode"], response)
        return response
    ...
    def flatten_raw(self):
        ...
        for data_key, data_type, type_default in [
            ...
            for property_ in self.data[data_key]:
                ...
                assert flat_key not in flattened, (
                    "duplicate key %s being added to flat" % flat_key
                )
    ...
    def flatten_sanitized(self):
        flattened = dict()
        for key in ["ContactID", "EmailAddress", "Name"]:
            flattened[key] = self.data.get(key)
